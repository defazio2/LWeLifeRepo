...
// 20171121 added to facilitate creation and use of the scdb for shuffling shenanigans
string /G scydb_n = "sc_y_database"
string /G scxdb_n = "sc_x_database"
...
// 20171023 changed
string long_wn = datecodegrep2(basename)
...
// 20171021 THIS IS CAUSING ISSUES WITH NAMES BEING TOO LONG FOR CLUSTER
	...
	do // loop over the events in _ptb
		...
		// copy just the event out of the raw wave
			...
			if( total_event_count == 0 )
				...
				//20171121 added to build database for shuffling stuff
				duplicate/O event_xw, $scxdb_n
				duplicate/O event_yw, $scydb_n
			else
				concatenate/NP {event_yw}, out_yw // np means no promotion of dimensions
				concatenate/NP {event_xw}, out_xw		
				
				// 20171121 added to build database for shuffling stuff
				concatenate {event_yw}, $scydb_n
				concatenate {event_xw}, $scxdb_n
			endif
			total_event_count += 1
			thiseventcount += 1
