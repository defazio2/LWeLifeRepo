...
////////////////////////////////////////////////
////////////////////////////////////////////////
function/S panalSuperFly( series_ave, win, pathn, expcode, subwaven, [panelDisplay] )
variable series_Ave // 1 means average sweeps, 0 means analyze each sweep/trace
string win, pathn, expcode, subwaven
variable panelDisplay // set to 1 to use with passive panel, set to 0 to use BPD
...
// clean up
close refnum
cleanupwaves(1) // kills all waves 
...
if(ipass>1)
	...
	if( paramisdefault( panelDisplay ) )
		bpd( expcode = expcode )
	endif
endif
...
if(waveexists( W_StatsTTest))
	...
	sig = 0
	print "returnTTest: failed StatsTTEST", mean(region1), mean(region2)
endif
...
if( secs == 0 )
	print "pmsecs2igor zero time!", secs
	newsecs = NaN
endif
...
//	string wavelist=tracenamelist("",";",1)
	...
	do
			...
			// TESTS IF THERE'S A SPIKE ANYWHERE IN BASELINE OR STEADY STATE
...
// 2018 sept 27 revised to assume 20 msec, warn if not!
...
//if(delta<0.001)
if( delta != 0.02 )
	//print "***  AUTO-DETECT PULSE not equal to 20 msec. Non standard interval? USER VERIFY!! ***"
	delta=0.02
endif
return delta
end
