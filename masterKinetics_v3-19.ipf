...
//				displaywavelist2subwin( tnlist, target, kill=1, xwaven=xtn )
				...
				if( ntraces > 0 )
					...
					if(strlen(range)>0)
						displaywavelist2subwin( tnlist, target,  kill=kill, svR = range )
					ELSE// this kill = 1, now kill=kill =0, to enable repeat rebuildmkp 20181207
						displaywavelist2subwin( tnlist, target,  kill=kill ) //, svR = range )
...
//
////////////////////
Function mkp_puExpListOLD(s) : PopupMenuControl
...
////////////////////
// 
// mkp_ popup EXP LIST : NOW ENABLES TAB SELECTION! mkpFOO
//
////////////////////
...
//	PopupMenu puRealExpList pos={posx,posy}, title="exp",proc=mkp_puExpList,mode=2, userdata=udata  // this is the target for the selection
	...
	if(s.eventcode == 2)
		...
		string expcode = S_Value
		if( strlen( expcode ) < 9 )
			print "mkp_plotProc: skipping bad exp code", expcode
			//debugger
		else
				...
				if( seriesnumber( selectedseries ) > 0 )
					string udata = s.userdata
	...
	// open expcode .dat file
		...
		if( V_flag == 0 )
			...
			if( V_flag == 0 )
				...
				print S_UserData
				//print "trying to keep on keeping on...."
				abort
			endif
		endif
...
// remove noise spikes
		variable dthresh = 2e-6 // replaces points exceeding 1uA/sec
		string temp_wl
...
//20180521		peakg = updateAct( delta_wl, tstart, tstart + twin, suffix,  do_autoV = "y", do_tau="_ptau" ) // returns the conductance and the boltz fit waves
		peakg = updateAct( temp_wl, tstart, tstart + twin, suffix,  do_autoV = "y", do_tau="_ptau" ) // returns the conductance and the boltz fit waves
...
//			string susg = updateAct( sub_wl, tend-tdur, tend, suffix, do_avg="doavg" ) // returns the conductance and the boltz fit waves
			string susg = updateAct( temp_wl, tend-tdur, tend, suffix, do_avg="doavg" ) // returns the conductance and the boltz fit waves
...
//		norminact = updateinact( sub_wl, tstart, tstart + twin, suffix, doautov="y")
		norminact = updateinact( temp_wl, tstart, tstart + twin, suffix, doautov="y")
...
// get raw list name
		...
		if( v_flag == 0 )
			print "failed to name the tabcontrol:", tabctrlname
			abort
		endif 
		string FOOudata = S_userData
		string tabname = stringbykey( "CurrentTab", FOOudata )
		...
		if( strlen( udata ) < 1 )
			debugger
			print "SIRI: failed to get named userdata.", "cn", s.ctrlname, "tn", tabname, "ud", udata
...
//		open /R/P=$pathn refnum as filename
	...
	// get raw WAVE/Z list
		...
		controlinfo $puRAW
		if( V_Flag == 0 )
			print "in MKP SIRI failed to locate puRaw", puRaw
			debugger
		endif
...
// remove noise spikes
		...
		temp_wl = spikeremover( sub_wl, dthresh, "", ignore=tstart )		
// 20180521
		sub_wl = temp_wl // actually start using spikeremover?

		variable isweep = 0, nsweeps = itemsinlist( timinglist )
		variable SIRItest = nsweeps, order = 0 // if subtrace is last sweep, this is SI, otherwise Recovery from Inact; the last trace is fully inactivated in steady state inactivation
// default order of peak search is last sweep first, to switch order, set order = 1
		if ( SIRItest != subtrace ) // 20180521 this is a crude test to determine wheter or not to run LAG ANALYSIS. assumes subtracting last trace is inact
			// not subtracting last trace, so this must be recovery. use default reverse peak search
			...
			pks = measurepeak( sub_wl, tstart, tstart+twin, nsmth, "_SIRI", order = 1 ) //,  do_tau="tiTau" )  //updateinact( sub_wl, tstart, tstart + twin, suffix, doautov="nope")
		endif
//		pks = measurepeak( sub_wl, tstart, tstart+twin, nsmth, "_SIRI" ) //,  do_tau="tiTau" )  //updateinact( sub_wl, tstart, tstart + twin, suffix, doautov="nope")
		// normalize
		string npks = pks + "_n"
...
//		variable SIRItest = nsweeps // if subtrace is last sweep, this is SI, otherwise Recovery from Inact; the last trace is fully inactivated in steady state inactivation
		if ( SIRItest == subtrace ) // 20180521 this is a crude test to determine wheter or not to run LAG ANALYSIS.
			// SI
			newkey = Inact
...
string keylist = TC + ";" + perm + ";" + SSAn + ";" + SSIn + ";" + Inact + ";" + RecIn + ";" 
	...
	if(bs.eventcode == 2)
		string buttctrlname = bs.ctrlname
		string winn = bs.win 
		string pathn = getpath()
		print "bu2qubproc: pathn", pathn 
		pathinfo $pathn 
		print "user path info", v_flag, s_path
		if( v_flag == 0 )
			print "failed to create/set path:", pathn 
			abort
		endif
		...
		print "after first copyfolder:", s_path
		...
		for( itab = 0; itab < ntabs; itab += 1 )
			...
			if( (stringmatch( tabname, "*median*" )==0) && (stringmatch( tabname, "*mean*")==0) ) // skip the median tab!
				...
				copyfolder/O/P=$pathn as destfold
				print "mkp2qub, created new folder:", destfold, s_path

				pathstring =  s_path // parsefilepath(1,s_filename, ":",1,0)
				newPath /O $temp_path pathstring

				klist = getuserdata( winn, buttctrlname, tabname ) 
				//
				// HEAVY LIFTING
...
// TC in  2QuB
	...
	// this means that the TC waves already exist
		...
		variable sweep = 1, vhold = 0
		vstep = membranepotential( tn, sweep, tstart + 0.001 )
		tn = removequotes( stringfromlist( 1, tl, "," ) )
		sweep = 2
		vhold = membranepotential( tn, sweep, tstart+0.001) 
		vdelta = vhold - vstep
		if( vdelta < 0.0099 )
			print "()()()() VDELTA ()()()()() ", tn, vstep, vhold, vdelta
			//debugger
		endif
...
string pathname		// pass an Igor pathname 
	...
	pathinfo $pathn 
	if( v_flag == 0 )
		print "failed get path (get path), pathn: ", pathn
		abort
	else 
		print "succeeded in getting path (get path), ", pathn, s_path
	endif
